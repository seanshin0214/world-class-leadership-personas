...
import {
  ...
  updatePersonaSchema,
  deletePersonaSchema,
  suggestPersonaSchema,
  chainPersonasSchema,
  browseCommunitySchema,
  installCommunityPersonaSchema,
  validatePersonaName,
...
interface CommunityPersona {
  name: string;
  file: string;
  [key: string]: string; // 메타데이터 (author, category 등)
}
...
// 페르소나 디렉토리 초기화
async function initPersonaDir(): Promise<void> {
  try {
...
// 사용 기록 추가
async function trackUsage(personaName: string, context = ''): Promise<void> {
...
// 커뮤니티 페르소나 읽기
async function readCommunityPersona(name: string): Promise<string> {
  const validatedName = validatePersonaName(name);
